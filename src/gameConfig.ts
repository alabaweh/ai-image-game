...
// Store the original image arrays
const originalLevels: Level[] = [
    {
        id: 1,
...
// Function to get a shuffled version of the levels
export const getShuffledLevels = (): Level[] => {
    return originalLevels.map(level => ({
        ...level,
        images: shuffleArray([...level.images])
    }));
};

// Export the game config with shuffled levels
...
export const createLevel = (id: number, title: string, images: ImageData[]): Level => {
    ...
    // Shuffle the images array to randomize positions
